...
async function publishArticleInBackground(body: BlogWebhookPayload) {
  ...
  // Strip any "Article generated by BabyLoveGrowth" attribution and links to
  // babylovegrowth.ai before persisting. Language-agnostic patterns.
  const stripAttribution = (html: string): string => {
    if (!html) return html;
    return html
      .replace(
  ...
  const row = {
    ...
    content_en: stripAttribution(content),
    content_it: stripAttribution(it.content),
    content_fr: stripAttribution(fr.content),
    content_de: stripAttribution(de.content),
    content_es: stripAttribution(es.content),
